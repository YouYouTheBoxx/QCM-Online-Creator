...
<button class="hidd customargr" id="submit" style="background-color: red;" >Save QCM</button> 
</div>
<div>
<div class="quiz-container">
  <div id="quiz"></div>
...
<script type="text/javascript">
        ...
        const answers = [];
			...
			output.push(
			  `<div class="slide">
				...
				<button class="hidd customargr" id="edit" value="enedit">Edit Question</button>
			  </div>`
			);
      }
    );
  ...
  function startEdit(currentIndex){
	shadowCurrentSlide = currentSlide;
	currentSlide = currentIndex;
	editQuestion();
	allqContainer.style['display'] = "none";
  ...
  function updateSuperSlide(){
	const selectorb = `.lquestion${myQuestions[currentSlide].id}`;
	const alllabelsb = document.querySelectorAll(selectorb);
	const selector = `.slquestion${myQuestions[currentSlide].id}`;
	const alllabels = document.querySelectorAll(selector);
	const selectorIn = `#si${myQuestions[currentSlide].id}`;
	const allinputs = document.querySelectorAll(selectorIn);
	const allinputsCheck = document.querySelectorAll(`#i${myQuestions[currentSlide].id}:checked`);
	const titlb = document.querySelector(`label[id='tquestion${myQuestions[currentSlide].id}']`);
	const titl = document.querySelector(`label[id='stquestion${myQuestions[currentSlide].id}']`);
	titl.textContent = titlb.textContent;
	allinputsCheck.forEach(function(userItem) {
		allinputs.forEach(function(auserItem) {
			if (auserItem.value === userItem.value){
				auserItem.checked = true;
			}
		});
	});
	var qidv = 0;
	alllabelsb.forEach(function(userLabel) {
		alllabels[qidv].textContent = userLabel.textContent;
		qidv++;
	});
	currentSlide = shadowCurrentSlide;
  }
  ...
  function addSuperSlide(){
    ...
    // for each question...
		...
		const currentQuestion = myQuestions[currentSlide];
			...
			for(letter in currentQuestion.answers){
			  ...
			  // ...add an HTML radio button
				  ...
				  answers.push(
					...
					<input type="radio" class="form-input-checkbox" name="squestion${currentQuestion.id}" id="si${currentQuestion.id}" value="${letter}" disabled="true"></input>
					<span class="lock" style="font-size: 1.25em;margin-right: 1em;cursor: default;" id="sis${currentQuestion.id}"></span>
					<label name="slquestion${currentQuestion.id}" id="slquestion${currentQuestion.id}${letter}" class="overlay2 slquestion${currentQuestion.id}" style="margin: 0;text-align: center;min-width: 12em;line-height: 1.5em;min-height: 1.5em;opacity: 1;user-select: none;cursor: default;" value="${letter}" for="" contenteditable="false">
					  ${currentQuestion.answers[letter]}
					</label>
					</label>
					</span><br><br>`
				  ...
				  answers.push(
					`<span>
					<label class="container" style="display: flex;cursor: default;">
					<input type="checkbox" class="form-input-checkbox" name="squestion${currentQuestion.id}" id="si${currentQuestion.id}" value="${letter}" disabled="true"></input>
					<span class="lock" style="font-size: 1.25em;margin-right: 1em;cursor: default;" id="sis${currentQuestion.id}"></span>
					<label name="slquestion${currentQuestion.id}" id="slquestion${currentQuestion.id}${letter}" class="overlay2 slquestion${currentQuestion.id}" style="margin: 0;text-align: center;min-width: 12em;line-height: 1.5em;min-height: 1.5em;opacity: 1;user-select: none;cursor: default;" value="${letter}" for="" contenteditable="false">
					  ${currentQuestion.answers[letter]}
					</label>
				  ...
				  answers.push(
					`<span>
					<label class="container" style="display: flex;cursor: default;">
					<input type="radio" class="form-input-checkbox" style="display:none;" name="squestion${currentQuestion.id}" id="si${currentQuestion.id}" value="${letter}" checked="true" disabled="true">
					<label name="slquestion${currentQuestion.id}" id="slquestion${currentQuestion.id}${letter}" class="overlay2 slquestion${currentQuestion.id}" style="margin: 0;text-align: center;min-width: 12em;line-height: 1.5em;min-height: 1.5em;opacity: 1;user-select: none;cursor: default;" value="${letter}" for="" contenteditable="false">
					  ${currentQuestion.answers[letter]}
					</label>
					</label>
			...
			// add this question and its answers to the output
			output.push(
			  `<div class="overlay2" style="opacity: 1;" name="s${currentQuestion.id}">
				<div name="stquestion${currentQuestion.id}" class="question stquestion${currentQuestion.id}" contenteditable="false"><label id="stquestion${currentQuestion.id}" class="overlay2" style="min-width: 0.125em;opacity: 1;user-select: none;cursor: default;">${currentQuestion.question}</label></div>
				<div class="answers"> ${answers.join("")} </div><br>
				<button class="customargr" style="background-color: red;" id="edtquest" value="${currentSlide}" onclick="startEdit(this.value)">Edit Question</button>
			  </div>`
			);
  ...
  function addSlide(){
    ...
    // for each question...
	var currentQuestion = myQuestions[idn-1];
		const answers = [];
			...
			for(letter in currentQuestion.answers){
			  ...
			  if (currentQuestion.type == "unique" || currentQuestion.type == "real"){
				  answers.push(
					...
					<label name="lquestion${currentQuestion.id}" id="lquestion${currentQuestion.id}${letter}" class="overlay2 lquestion${currentQuestion.id}" style="margin: 0;text-align: center;min-width: 12em;line-height: 1.5em;min-height: 1.5em;opacity: 1;border: none;" value="${letter}" for="" contenteditable="true">
					  ${currentQuestion.answers[letter]}
					</label>
					</label>
					</span><br><br>`
			...
			output.push(
			  `<div class="slide" name="${currentQuestion.id}">
				<div name="tquestion${currentQuestion.id}" class="question tquestion${currentQuestion.id}" contenteditable="true"><label id="tquestion${currentQuestion.id}" class="overlay2" style="min-width: 0.125em;opacity: 1;">${currentQuestion.question}</label></div>
				<div class="answers"> ${answers.join("")} </div>
			  </div>`
			);
  ...
  function editQuestion() {
	...
	const masterdiv = document.querySelector(`div[name='${myQuestions[currentSlide].id}']`);
	...
	if (editButton.value == "enedit"){
		...
		allqContainer.style['display'] = "inline-block";
  ...
  const allqContainer = document.getElementById('qctd');
  ...
  let shadowCurrentSlide = 0;
  let currentSlide = shadowCurrentSlide;
